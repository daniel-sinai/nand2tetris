...
def assemble_file(
    ...
    *Second Pass*
    ...
    while first_parser.has_more_commands():
        ...
        if first_parser.command_type() == "A_COMMAND" or \
                first_parser.command_type() == "C_COMMAND":
            command_line = command_line + 1
        if first_parser.command_type() == "L_COMMAND":
            symbol_table.add_entry(first_parser.symbol(), command_line)
    ...
    while second_parser.has_more_commands():
        ...
        if command_type == "A_COMMAND":
            ...
            if symbol.isdigit():
                ...
                symbol_table.add_entry(symbol, next_available)
                next_available += 1
            if symbol_table.contains(symbol):
                address = symbol_table.get_address(symbol)
            output_file.write("0" + str(bits(address)).zfill(15) + '\n')
        elif command_type == "C_COMMAND":
            if ">>" in second_parser.comp() or "<<" in second_parser.comp():
                output_file.write("101")
            else:
                output_file.write("111")
            output_file.write(code.comp(second_parser.comp()) +
