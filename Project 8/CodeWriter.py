...
class CodeWriter:
    ...
    def write_function(self, function_name: str, n_vars: int) -> None:
        ...
        for i in range(n_vars):
            self.output.write(self.__push_to_stack("0"))

    def __save_pointers(self, pointer):
        to_write = "@" + pointer + "\nD=M\n" + self.__push_to_stack("D")
        return to_write
    ...
    def write_call(self, function_name: str, n_args: int) -> None:
        ...
        to_write = "// call " + function_name + " " + str(n_args) + "\n"
        to_write += "@" + self.filename + ".RETURN" + str(
            self.function_counter) + "\nD=A\n" + \
            self.__push_to_stack("D")
        to_write += self.__save_pointers("LCL") + \
            self.__save_pointers("ARG") + \
            self.__save_pointers("THIS") + \
            self.__save_pointers("THAT")
        to_write += "@SP\nD=M\n@" + str(5 + n_args) + "\nD=D-A\n@ARG\nM=D\n" \
                                                      "@SP\nD=M\n@LCL\nM=D\n"
        self.output.write(to_write)
        self.write_goto(function_name)
        self.write_label(
            self.filename + ".RETURN" + str(self.function_counter))

    def __recover_pointers(self, arg):
        to_write = "@R14\nAM=M-1\nD=M\n@" + arg + "\nM=D\n"
        return to_write
    ...
    def write_return(self) -> None:
        ...
        to_write = ""
        # Calculating endFrame and save it in R14
        to_write += "@LCL\nD=M\n@R14\nM=D\n"
        # Getting the return address and saves it in R15
        to_write += "@5\nD=A\n@R14\nD=M-D\nA=D\nD=M\n@R15\nM=D\n"
        self.output.write(to_write)
        self.write_push_pop("C_POP", "argument", 0)  # POP TO ARG[0]
        # SP=ARG+1
        to_write = "@ARG\nD=M\n@SP\nM=D+1\n"
        # Recover pointers
        to_write += self.__recover_pointers("THAT")
        to_write += self.__recover_pointers("THIS")
        to_write += self.__recover_pointers("ARG")
        to_write += self.__recover_pointers("LCL")
